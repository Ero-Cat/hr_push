...
const String heartRateMeasurementUuidFull =
...
class BleHeartRateService {
  ...
  Stream<AdapterConnectionState> get connectionStateStream =>
  ...
  Future<void> startScan() async {
    ...
    _scanSubscription = _adapter.scanStream.listen((device) {
      ...
      final hasHrService = device.serviceUuids.any(
        (uuid) =>
  ...
  Future<void> connect(String deviceId) async {
    ...
    _connectionSubscription = _adapter.connectionStateStream(deviceId).listen((
      state,
    ) {
    ...
    try {
      ...
      _valueSubscription = _adapter
          .valueStream(deviceId, hrService.uuid, hrChar.uuid)
          .listen((data) {
  ...
  HeartRateData? _parseHeartRateData(Uint8List data) {
    ...
    final sensorContactDetected = sensorContactSupported
        ? (flags & 0x02) != 0
        : true;
    ...
    return HeartRateData(bpm: bpm, sensorContact: sensorContactDetected);
  }
