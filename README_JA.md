...
- **デスクトップ最適化**：Windows/macOS/Linux で縦長固定。Windows はトレイに最小化してバックグラウンド推送を継続。
- **セットアップ**：初回ウィザード、入力バリデーション、HTTP/WS/OSC/MQTT のワンタップ接続テスト。
- **iOS ライブアクティビティ**：ロック画面/Dynamic Island に心拍数を表示（iOS 16.1+）。
...
心拍数イベントには `heartRate` と `heart_rate` の両方のキー（同値）が含まれます。前者は本ドキュメント、後者は旧バージョン互換用です。
...
| MQTT ユーザー/パス | 任意 | 空 |
| MQTT Client ID | 空ならデフォルト ID | 空 |
| MQTT TLS | mqtts を有効化（通常 8883） | オフ |
| MQTT LWT トピック | 異常切断時にオフラインメッセージを発行（任意） | 空 |
| 最大心拍 | 率計算用 | `200` |
...
2. Xiaomi Smart Band 9/10、Redmi Watch シリーズ（本体で「心拍数ブロードキャスト」を有効化してから使用）
...
## 🧾 更新履歴
### v1.8.0
- **Redmi/Xiaomi ウォッチ互換性**：Redmi 端末や無名の心拍ブロードキャスト機器をスキャン対象に追加。接続タイムアウト修正、再接続ストームの解消（指数バックオフ + 連続失敗上限）、無データ接続の検出、MAC ローテーションへの名前ベース再接続。心拍数サービスが見つらない場合は「心拍数ブロードキャスト」設定ガイドを表示（3 言語）。
- **設定ページ再構築**：プロトコル別グループ化・開閉式詳細設定、全項目のインラインバリデーション、HTTP/WS/OSC/MQTT のワンタップ接続テスト、未保存変更の保護、保存フィードバック、OSC ステータス表示、初回ウィザード。
- **MQTT 拡張**：TLS (mqtts/8883)、パスワード表示切替、LWT トピック、安定したデフォルト Client ID。
- **プッシュ性能**：HTTP の持続接続再利用、プロトコル並列送信（遅いエンドポイントが VRChat OSC を妨げない）、WebSocket 指数バックオフ再接続、OSC DNS キャッシュ、`hr_connected` は状態変化時のみ送信。
- **データ形式**：payload に `event/connected/device` を追加し `heart_rate` 互換キーを導入。接続/切断イベントも HTTP/WS/MQTT へ配信。
- **新機能**：iOS ライブアクティビティ（ロック画面 / Dynamic Island、iOS 16.1+）、Windows トレイ最小化でバックグラウンド継続。
- **体験とアクセシビリティ**：ステータス文言の完全 3 言語化（Android 通知含む）、システム文字サイズ対応、セマンティクスラベル、心拍アニメーションの描画最適化。
- **安定性**：Android スキャン制限の自己回復、近隣リスト 15 件への拡張とちらつき防止、リソース解放の修正。未使用モジュール 5 件削除、テスト 25 → 51 件。

### v1.6.1
- **Android の最適化**：Proguard ルールを更新し、ビルドの難読化を最適化しました。
- **BLE アダプターの微調整**：`universal_ble` アダプターレイヤーを微調整し、接続の安定性を向上させました。
- **Windows ビルドの強化**：C++ 標準を 20 にアップグレードし、コンパイル警告を修正しました。CI が Windows ビルド産物を自動的にアップロードするようになりました。
...
### 🔴 Xiaomi / Redmi ウォッチ（重要）

Xiaomi / Redmi ウォッチは既定で Xiaomi 独自の BLE プロトコル（サーバー経由のペアリングが必要）を使用し、標準の心拍数サービスを公開しません。HR PUSH で使用するには、ウォッチ本体で「心拍数ブロードキャスト」を有効にしてください：

1. ウォッチの **設定 → 心拍数** で **心拍数ブロードキャスト** をオンにします。
2. ウォッチが Mi Fitness アプリに接続されていないことを確認します（BLE は単一接続）。
3. HR PUSH で再スキャンして接続します。

有効化すると、ウォッチは標準の心拍数サービス（0x180D）をアドバタイズし、ペアリングなしで接続できます。接続後にサービスが見つからない場合は、アプリが自動で案内を表示します。
